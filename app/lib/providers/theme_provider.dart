...
class ThemeProvider extends ChangeNotifier {
  ...
  static const String _appColorThemeKey = 'app_color_theme';
  ...
  AppColorTheme _appColorTheme = AppColorTheme.defaults;
  ...
  AppColorTheme get appColorTheme => _appColorTheme;
  ...
  List<AppColorTheme> get availableAppColorThemes =>
      List<AppColorTheme>.unmodifiable(AppColorTheme.values);
  ...
  String get appColorThemeLabel => _appColorTheme.label;
  ...
  Future<void> _loadAppColorTheme() async {
    final prefs = await SharedPreferences.getInstance();
    final savedTheme = prefs.getString(_appColorThemeKey);
    final resolved = AppColorTheme.fromStorage(savedTheme);
    if (_appColorTheme == resolved) return;
    _appColorTheme = resolved;
    notifyListeners();
  }

  Future<void> setAppColorTheme(AppColorTheme theme) async {
    if (_appColorTheme == theme) return;
    _appColorTheme = theme;
    notifyListeners();
    final prefs = await SharedPreferences.getInstance();
    await prefs.setString(_appColorThemeKey, theme.storageValue);
  }
